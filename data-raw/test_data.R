...
readr::write_rds(mill_wells, "misc/mills_fixed.rds")

mill_wells <- wells_subset(mill, fix_bottom = FALSE)
mill_wells <- wells_elev(mill_wells, mill_lidar)
readr::write_rds(mill_wells, "misc/mills_unfixed.rds")

y <- wells_yield(mill_wells)


# Mini Wells for Package tests ------------------------------------
