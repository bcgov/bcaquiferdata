...
flags <- dplyr::tribble(
  ...
  "flag_int_missing"                                                                        , "Interval is missing lithologic record."                                                                                                                                                                                              , "Check original paper log"                                                                               ,
  "flag_int_overlap"                                                                        , "Interval overlaps the next or previous interval."                                                                                                                                                                                    , "Check original paper log, or fix in GWELLS if obvious"                                                  ,
  "flag_int_gap"                                                                            , "Interval has a gap between it and the next or previous interval."                                                                                                                                                                    , "Check original paper log, or fix in GWELLS if obvious"                                                  ,
  "flag_int_note"                                                                           , "This first depth interval has depth values both to and from 0 (or `NA`) which marks possible notes made before the lithology records."                                                                                               , "Check original paper log"                                                                               ,
  "flag_int_overrun"                                                                        , "Second or later depth interval has depth values both to and from 0 (or `NA). This marks an interval as a possible overrun, where the notes from a previous record have overrun onto the next line."                                  , "Check original paper log"                                                                               ,
  ...
  "flag_int_bottom"                                                                         , "Bottom interval with zero depth. Either because `to` is 0 (or `NA`) or because `to` == `from`."                                                                                                                                      , "Use `fix_bottom` argument in `wells_subset()` to add 1m to this bottom interval (this is the default)." ,
  ...
  "flag_lith_nodepths"                                                                      , "All lithologic intervals have depths of 0 (or `NA`) in both `from` and `to`."                                                                                                                                                        , "Check original paper log"                                                                               ,
  ...
  # Yield flags
  "flag_yield_mismatch"                                                                     , paste0("Lithology where there are both depths and yields, ", "but the number of yield measures do not match up with the number of depth measures ", "(thus `yield` and `depths` are `NA`). This only applies to Hydrostratigraphy.")  , "Check original paper log"                                                                               ,
  ...
  # Well depth
  "flag_depth_missing"                                                                      , "Well is missing depth."                                                                                                                                                                                                              , "Check original paper log"                                                                               ,
  ...
  "flag_depth_mismatch"                                                                     , "Well depth is not equal to the depth of the final lithology interval."                                                                                                                                                               , "Check original paper log"                                                                               ,
) |>
