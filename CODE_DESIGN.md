...
- Descriptions for parameter arguments should follow:
  ...
  - e.g.,
    `@param region sf simple features object. Shape file of the region of interest.`

- Document repetitive arguments (ones found in more than one function)
  in the `R/aa_common_docs.R` file (named to sort to the top of the
  folder), and use `@inheritParams common_docs` in the function
  documentation. This way duplicate documentation stays consistent.
...
- TRIM DEM
  ...
  via [`bcmaps::cded`](http://bcgov.github.io/bcmaps/reference/cded.md)
- Lidar DEM
