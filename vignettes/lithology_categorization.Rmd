...
lith_tbl <- function(d, colour = TRUE) {
  ...
  d <- d |>
    ...
    mutate(across(contains("flag_"), \(x) if_else(x, "TRUE", ""))) |>
    gt() |>
    tab_spanner(
      label = md(
        "**Intermediate Categories**<br><small>(`lith_primary`,`lith_secondary`,`lith_tertiary`)</small>"
      ),
    ...
    cols_label(
      lithology_raw_combined = md(
        "**Starting Description**<br><small>(`lithology_raw_combined`)</small>"
      ),
      lithology_clean = md(
        "**Cleaned Description**<br><small>(`lithology_clean`)</small>"
      ),
      lith_primary = md("1°"),
      lith_secondary = md("2°"),
      lith_tertiary = md("3°"),
      lithology_category = md(
        "**Final Category**<br><small>(`lithology_category`)</small>"
      ),
...
```{r, echo = FALSE}
lith_fix(
  desc = c("sand", "sand with silt", "silty sand", "sand with silty clay")
) |>
  lith_tbl() |>
...
```{r}
lith_fix(
  desc = c("fractured bedrock", "fractured", "fractured bedrock and sand")
) |>
  lith_tbl() |>
...
lith_fix(
  ...
  tab_footnote(
    footnote = "Example of similar terms but different category",
...
```{r}
lith_fix(
  desc = c("organic", "organic with sand", "sandy organic", "organic and sand")
) |>
  lith_tbl() |>
  tab_source_note(
    md(
...
```{r}
lith_fix(
  desc = c("gravel", "sand", "clay", "silt", "silty clay", "clay with silt")
) |>
  lith_tbl()
```
...
lith_fix(
  ...
  tab_footnote(
    footnote = "Example of similar terms but different category",
    locations = cells_body(columns = 1, rows = c(last_col() - 1, last_col()))
  )
```
