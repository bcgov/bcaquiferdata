...
lidar_fetch <- function(
  ...
  if (only_new) {
    local_tiles <- list.files(out_dir)
  } else {
    local_tiles <- vector()
  }
  ...
  if (nrow(fetch) == 0) {
    stop(
      "Region does not overlap any BC tiles. ",
      "Are you sure this is a region in British Columbia?",
      call. = FALSE
    )
  }
  ...
  if (any(!fetch$tif_good)) {
    ...
    if (nrow(fetch) == 0) {
      stop(
        "No non-missing lidar tiles available, try TRIM data\n",
  ...
  for (i in seq_len(nrow(fetch))) {
    if (!quiet) {
      msg <- paste0("Fetching ", fetch$tile_name[i])
    }
