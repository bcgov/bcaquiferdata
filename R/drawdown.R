...
drawdown <- function(
  ...
  aquifers <- dd_aquifers(
    wells$aquifer_id[wells$focal],
    wells$aquifer_id,
    update
  )
  ...
  if (is.null(file_name)) {
    if (length(location) == 2) {} else {
      location <- paste0("well_", location)
    }
...
dd_wells <- function(location, update = FALSE) {
  ...
  wells |>
    dplyr::mutate(
      static_water_m = units::set_units(
        .data$static_water_level_ft_btoc,
        "ft"
      ) |>
        units::set_units("m"),
      ...
      bedrock_depth_m = units::set_units(.data$bedrock_depth_ft_bgl, "ft") |>
        units::set_units("m"),
      transmissivity_m_2_s = units::set_units(
        .data$transmissivity_m_2_s,
        "m2/s"
      ),
      transmissivity_m_2_d = units::set_units(
        .data$transmissivity_m_2_s,
        "m2/d"
      )
    ) |>
    ...
    dplyr::mutate(
      screen_depth = NA,
      url = paste0(
        "https://apps.nrs.gov.bc.ca/gwells/well/",
      ...
      aquifer_lithology_reassigned = dplyr::case_when(
        is.na(.data$bedrock_depth_m) &
          is.na(.data$aquifer_lithology_code) ~ "Unassigned",
        is.na(.data$bedrock_depth_m) ~ .data$aquifer_lithology_code,
        .data$well_depth_m - .data$bedrock_depth_m >
          units::set_units(5, "ft") ~ "Bedrock",
        TRUE ~ "Unconsolidated"
      )
    ) |>
...
dd_wells_testing <- function(wells, update = FALSE) {
  data_read("wells_testing", update) |>
    dplyr::right_join(
      dplyr::select(wells, "well_tag_number", "aquifer_id"),
      by = "well_tag_number"
    ) |>
...
dd_trans_store <- function(focal, update = FALSE) {
  ...
  if (!inherits(focal, "sfc")) {
    message("Well missing transmissivity and/or storativity")
  }
  ...
  if (nrow(aq) > 1) {
    message(
      "  `location` overlaps more than one aquifer (",
      paste0(aq$aquifer_id, collapse = ", "),
      ") using aquifer ",
      aq$aquifer_id[1]
    )
    aq <- aq[1, ]
  ...
  if (nrow(aq) == 0 || is.na(aq$subtype)) {
    stop(
      "Cannot find aquifer or missing aquifer subtype.",
      " Supply `transmissivity` and `storativity` manually.",
      call. = TRUE
    )
  }
...
dd_drawdown <- function(wells, locs) {
  ...
  dd <- dd |>
    dplyr::mutate(
      final_depth = paste0(
        "=IF(",
        eloc(dd, "screen_depth"),
        "<>\"\", ",
        eloc(dd, "screen_depth"),
        ", ",
        eloc(dd, "well_depth_m"),
        ")"
      ),
      drawdown = paste0(
        "Inputs!",
      ...
      impact = glue::glue(
        "=IF(ISNUMBER({eloc(dd, 'safe')}),{eloc(dd, 'drawdown')}/{eloc(dd, 'safe')}, \"\")"
      )
    ) |>
  ...
  # Apply formula class
  for (x in nms$name_nice[
    nms$name %in% c("final_depth", "drawdown", "safe", "impact")
  ]) {
...
dd_sheet_inputs <- function(wb, location, inputs, locs, space, s = "Inputs") {
  ...
  openxlsx::writeData(
    wb,
    s,
    x = "Calculation of impact to adjacent wells from a pumping well"
  )
  openxlsx::addStyle(
    wb,
    s,
  ...
  openxlsx::writeData(
    wb,
    s,
    x = "'Distance to Well' in Drawdown worksheet",
    startCol = ncol(inputs),
    startRow = nrow(inputs) + space + 1
  )
  openxlsx::addStyle(
    wb,
    s,
    col = ncol(inputs),
    row = nrow(inputs) + space + 1,
    style = s_it("left")
  )
  openxlsx::addStyle(
    wb,
  ...
  openxlsx::addStyle(
    ...
    rows = space +
      1 +
      which(is.na(inputs$Value[inputs$Parameter != "Distance"])),
    stack = TRUE
  )
  ...
  openxlsx::writeData(
    wb,
    s,
    x = "Equations for calculating Drawdown",
    startCol = 1,
    startRow = space + 1 + nrow(inputs) + 1 + 1
  )
  openxlsx::mergeCells(
    wb,
    s,
    cols = 1:5,
    rows = space + 1 + nrow(inputs) + 1 + 1
  )
  openxlsx::addStyle(
    wb,
    s,
  ...
  openxlsx::writeData(
    ...
    x = eqs,
    startCol = 1,
    startRow = space + 1 + nrow(inputs) + 1 + 1 + 1
  )
  openxlsx::addStyle(
    wb,
    s,
    style = s_head(),
    cols = seq_len(ncol(eqs)),
    rows = space + 1 + nrow(inputs) + 1 + 1 + 1,
    stack = TRUE
  )
  openxlsx::addStyle(
    wb,
    s,
    style = openxlsx::createStyle(numFmt = "0.00000"),
    cols = ncol(eqs),
    rows = space + 1 + nrow(inputs) + 1 + 1 + 1 + 1,
    stack = TRUE
  )
  purrr::map(
    seq(space + 1 + nrow(inputs) + 1 + 1 + 1, length.out = nrow(eqs) + 1),
    \(x) openxlsx::mergeCells(wb, s, cols = 2:3, rows = x)
  )
  openxlsx::writeData(
    wb,
    s,
    x = "'Drawdown Impact' in Drawdown worksheet",
    startCol = 4,
  ...
  openxlsx::addStyle(
    ...
    s,
    col = 4,
    row = space + 1 + nrow(inputs) + 1 + 1 + 1 + 3,
    style = s_it("left")
  )
...
dd_sheet_drawdowns <- function(wb, dd, s = "Drawdown") {
  ...
  s_aq_focal <- openxlsx::createStyle(
    bgFill = "#afd095",
    fontSize = 10,
    halign = "center"
  )
  s_aq_diff <- openxlsx::createStyle(
    bgFill = "#b4c7dc",
    fontSize = 10,
    halign = "center"
  )
  s_aq_na <- openxlsx::createStyle(
    bgFill = "#ec9ba4",
    fontSize = 10,
    halign = "center"
  )
  ...
  openxlsx::conditionalFormatting(
    ...
    s,
    style = s_aq_focal,
    cols = cols$col_n[cols$name == "aquifer_id"],
    rows = rows,
    rule = paste0(aid, "2==$", aid, "$2"),
    stack = TRUE
  )
  openxlsx::conditionalFormatting(
    wb,
...
dd_sheet_hydrogeologic <- function(
  wb,
  aquifers,
  wells_testing,
  s = "Hydrogeologic Setting"
) {
  # Add sheet data
  openxlsx::addWorksheet(wb, s)
  ...
  rows_wt <- seq(
    rows_aq[length(rows_aq)] + 3,
    length.out = nrow(wells_testing) + 1
  )
  ...
  openxlsx::writeData(
    wb,
    s,
    startRow = rows_wt[1] - 1,
    x = "Hydraulic Parameters from Well Testing"
  )
  ...
  openxlsx::addStyle(
    ...
    s,
    style = s_head(),
    cols = seq_len(ncol(wells_testing)),
    rows = rows_wt[1]
  )
  openxlsx::addStyle(
    wb,
  ...
  openxlsx::addStyle(
    ...
    s,
    style = s_wrap,
    cols = stringr::str_which(names(aquifers), "Depth"),
    rows = rows_aq[1],
    stack = TRUE
  )
  openxlsx::addStyle(
  ...
  openxlsx::addStyle(
    ...
    stack = TRUE,
    rows = rows_aq[-1],
    cols = cols_aq$col_n[cols_aq$name == "aquifer_details_url"]
  )
  openxlsx::addStyle(
    wb,
    s,
    ...
    stack = TRUE,
    rows = rows_wt[-1],
    cols = cols_wt$col_n[cols_wt$name == "comments"]
  )
...
dd_sheet_glossary <- function(wb, s = "Glossary") {
  ...
  openxlsx::addStyle(
    wb,
    s,
    row = c(startRow, startRow + nrow(g) + 2),
    col = 1:2,
    style = s_head(),
    gridExpand = TRUE
  )
...
dd_sheet_metadata <- function(wb, s = "Metadata") {
  ...
  openxlsx::writeData(
    ...
    startRow = startRow + 2,
    x = paste0(
      "Excel template created with the bcaquiferdata R package on ",
      Sys.Date()
    )
  )
  ...
  openxlsx::addStyle(
    wb,
    s,
    row = (startRow + 4):(startRow + 4 + nrow(meta)),
    col = 1,
